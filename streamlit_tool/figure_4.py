...
with psycopg.connect("dbname=project1 user=postgres password=postgres") as conn:
    ...
    with conn.cursor() as cur:
        total_sentiment_score_rankings = []
        ...
        with open("data.csv", 'r') as file_object:
            ...
            for line in file_object:
                ...
                if (espn_score_ranking < 1):
                    ...
                    # print((total_sentiment_score_ranking,espn_score_ranking))
                    total_sentiment_score_rankings.append(total_sentiment_score_ranking)
                    season_leader_rankings.append(espn_score_ranking)
                    colors.append("blue")

                
        # print((total_sentiment_score_rankings))
        # print((season_leader_rankings))
        fig = plt.figure(figsize=(12, 6))
        plt.scatter(total_sentiment_score_rankings, season_leader_rankings, c=colors, alpha=0.5, cmap='viridis')
        plt.gca().invert_xaxis()
        plt.gca().invert_yaxis()
        plt.xlabel("Total Sentiment Score Ranking",fontsize=20)
        plt.ylabel("Season Leader Rankings",fontsize=20)
        plt.title("Total Sentiment Score Ranking vs Season Leader Ranking",fontsize=18)
